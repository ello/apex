defmodule Ello.V2.LinkView do
  ...
  def render("links.json", links) do
    links
    |> Enum.map(fn link ->
      link
      |> sanitize_url
  ...
  defp match_url_to_icon_and_format(url) do
    icon_types
    |> Enum.find(fn(icon_type) -> Regex.match?(icon_type.regex, url) end)
    |> format_link(url)
  end
  ...
  defp social_icon_url(icon_type) do
    icon_type |> Map.put(:icon, "#{System.get_env("SOCIAL_ICONS_URL")}/#{icon_type.icon}")
  end
  ...
  defp icon_types do
    [
      ...
      %{type: "500px", icon: "500px.png", regex: ~r/(?:(?:http|https):\/\/)?(?:www.)?facebook.com\/(?:(?:\w)*#!\/)?(?:pages\/)?(?:[?\w\-]*\/)?(?:profile.php\?id=(?=\d.*))?([\w\-]*)?/},
      ...
      %{type: "Model Mayhem", icon: "model_mayhem.png", regex: ~r/http(?:s)?:\/\/(?:www\.)?mixcloud\.com\/([a-zA-Z0-9_]+)|mixcloud\.com\/([a-zA-Z0-9_]+)/},
      %{type: "Patreon", icon: "patreon.png", regex: ~r/http(?:s)?:\/\/(?:www\.)?patreon\.com\/([a-zA-Z0-9_-]+)|patreon\.com\/([a-zA-Z0-9_-]+)/},
      %{type: "Persona", icon: "persona.png", regex: ~r/http(?:s)?:\/\/(?:www\.)?patreon\.com\/([a-zA-Z0-9_-]+)|patreon\.com\/([a-zA-Z0-9_-]+)/},
