defmodule Ello.Search.UserSearchTest do
  ...
  test "username_search - scores more exact matches higher", context do
    results = UserSearch.username_search(context.user.username, %{current_user: context.current_user})
  ...
  test "username_search - does not include locked users", context do
    results = UserSearch.username_search("username", %{current_user: context.current_user})
  ...
  test "username_search - includes nudity users", context do
    results = UserSearch.username_search(context.spam_user.username, %{current_user: context.current_user})
    assert context.user.id in Enum.map(results, &(&1.id))
  ...
  test "username_search - does not include blocked users", context do
    ...
    results = UserSearch.username_search("username", %{current_user: current_user})
    assert context.user.id in Enum.map(results, &(&1.id))
  ...
  test "username_search - lana test", context do
    ...
    results = UserSearch.username_search("lana", %{current_user: context.current_user})
  ...
  test "user_search - does not include spamified users", context do
    results = UserSearch.user_search("username", %{allow_nsfw: false, allow_nudity: false, current_user: context.current_user})
    assert context.user.id == hd(Enum.map(results, &(&1.id)))
    refute context.spam_user.id in Enum.map(results, &(&1.id))
  end

  test "user_search - does not include locked users", context do
    results = UserSearch.username_search("username", %{allow_nsfw: false, allow_nudity: false, current_user: context.current_user})
    assert context.user.id in Enum.map(results, &(&1.id))
    refute context.locked_user.id in Enum.map(results, &(&1.id))
  end
  ...
  test "user_search - includes nudity users if client allows nudity", context do
    results = UserSearch.user_search("username", %{allow_nsfw: false, allow_nudity: true, current_user: context.current_user})
    assert context.user.id in Enum.map(results, &(&1.id))
    assert context.nudity_user.id in Enum.map(results, &(&1.id))
  end
  ...
  test "user_search - does not include blocked users", context do
    ...
    results = UserSearch.user_search("username", %{allow_nsfw: true, allow_nudity: false, current_user: current_user})
  ...
  test "user_search - pagination", context do
    ...
    results = UserSearch.user_search("username", %{allow_nsfw: true, allow_nudity: true, current_user: context.current_user, per_page: "2"})
    ...
    results = UserSearch.user_search("username", %{allow_nsfw: true, allow_nudity: true, current_user: context.current_user, page: "3", per_page: "2"})
