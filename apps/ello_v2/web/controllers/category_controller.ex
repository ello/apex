defmodule Ello.V2.CategoryController do
  ...
  def show(conn, %{"id" => id_or_slug}) do
    category = Discovery.category(standard_params(conn, %{id_or_slug: id_or_slug}))
    api_render_if_stale(conn, data: category)
  end

  defp categories(conn, %{"all" => _}),
    do: Discovery.categories(standard_params(conn, %{meta: true, inactive: true}))
  defp categories(conn, %{"meta" => _}),
    do: Discovery.categories(standard_params(conn, %{meta: true}))
  defp categories(conn, _),
    do: Discovery.categories(standard_params(conn))
end
